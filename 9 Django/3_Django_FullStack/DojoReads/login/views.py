...
#metodo que envía a página por defecto
def login(request):
    if 'user_id' in request.session:
        return redirect('books/')
    return render(request, 'registro.html')

#metodo que redirige a usuario para que se registre cuando ha ingresado datos incorrectos en el registro
def registrar(request):
    return render(request, 'registro.html')

#Login (metodo de inicio)
def inicio(request):
...
#Register (metodo registro )
# si datos de registro son incorrectos envía a método registrar
#si datos son correctos los almacena y redirecciona
